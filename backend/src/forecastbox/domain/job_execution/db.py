...
"""Persistence layer for JobExecution, with auth-scoped authorization.
...
from forecastbox.schemas.jobs import JobExecution, JobExecutionStatus
...
async def upsert_job_execution(
    *,
    ...
    If ``job_execution_id`` is omitted a fresh UUID is generated (attempt 1).
    If ``job_execution_id`` is supplied the next attempt number is derived from the
...
async def count_job_executions_by_experiment(experiment_id: str, *, auth_context: AuthContext) -> int:
    ...
    async def function(i: int) -> int:
        async with _jobs_module.async_session_maker() as session:
            query = select(func.count(func.distinct(JobExecution.job_execution_id))).where(
