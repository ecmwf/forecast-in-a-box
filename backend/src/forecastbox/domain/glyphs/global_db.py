...
def _to_global_glyph_record(row: GlobalGlyph) -> GlobalGlyphRecord:
    return GlobalGlyphRecord(
        global_glyph_id=GlobalGlyphId(str(cast(Any, row.global_glyph_id))),
    ...
    def function(i: int) -> GlobalGlyphRecord:
        with _jobs_module.sync_session_maker() as session:
            ...
            if existing is not None:
                glyph_id = GlobalGlyphId(str(existing.global_glyph_id))
                session.execute(
                    update(GlobalGlyph)
                ...
                session.commit()
                refreshed = session.execute(select(GlobalGlyph).where(GlobalGlyph.global_glyph_id == glyph_id)).scalar_one()
                return _to_global_glyph_record(refreshed)
            new = GlobalGlyph(
                global_glyph_id=GlobalGlyphId(str(uuid.uuid4())),
                key=key,
                value=value,
                public=public,
            ...
            session.add(new)
            session.commit()
            return _to_global_glyph_record(new)
...
def get_global_glyph(global_glyph_id: GlobalGlyphId, auth_context: AuthContext) -> GlobalGlyphRecord | None:
    ...
    return None if row is None else _to_global_glyph_record(row)
...
def list_global_glyphs(
    ...
    def function(i: int) -> list[GlobalGlyphRecord]:
        with _jobs_module.sync_session_maker() as session:
            ...
            return [_to_global_glyph_record(r[0]) for r in result.all()]
