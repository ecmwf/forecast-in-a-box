...
Uses an in-memory SQLite engine (monkeypatching _jobs_module.sync_session_maker)
...
def test_upsert_none_version_on_missing_row_raises(mem_session_maker: sessionmaker[Session]) -> None:
...
def test_get_plugin_state_missing(mem_session_maker: sessionmaker[Session]) -> None:
