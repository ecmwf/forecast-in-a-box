...
def mem_session_maker(monkeypatch: pytest.MonkeyPatch) -> Generator[sessionmaker[Session], None, None]:
    ...
    monkeypatch.setattr(blueprint_db._jobs_module, "sync_session_maker", maker)
    yield maker
    engine.dispose()


def test_soft_delete_plugin_template_marks_matching_rows(mem_session_maker: sessionmaker[Session]) -> None:
...
def test_soft_delete_plugin_template_does_not_affect_other_plugins(
