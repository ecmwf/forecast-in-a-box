...
export fiab__backend__launch_browser="false"
mkdir -p "$FIAB_ROOT"
PIDFILE="$FIAB_ROOT/backend.pid"
LOGFILE="$FIAB_ROOT/backend.log"

if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
    echo "backend already running with pid $(cat "$PIDFILE") (per $PIDFILE), refusing to start another one or to use it" >&2
    exit 1
fi
...
uvx --with httpx python "$HERE/run_backendCheck.py"
