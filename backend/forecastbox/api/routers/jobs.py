...
async def get_tasks() -> api.JobProgressResponse:
    ...
    response: api.JobProgressResponse = client.request_response(api.JobProgressRequest(job_id=job_ids), f"{SETTINGS.cascade_url}") # type: ignore
    if response.error:
        raise Exception(f"Job progress retrieval failed: {response.error}")
    
    response.progress.update({record['job_id']: '100' for record in job_records if record.status == "completed"}) # type: ignore
    return response

@router.get('/status/{job_id}')
async def get_status_of_job(job_id: JobId) -> api.JobProgressResponse:
    """Get progress of a job."""
    progress_response: api.JobProgressResponse =  client.request_response(api.JobProgressRequest(job_id=job_id), f"{SETTINGS.cascade_url}") # type: ignore
