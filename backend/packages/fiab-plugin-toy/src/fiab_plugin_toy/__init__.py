...
def compiler(partitions: DataPartitionLookup, block_id: BlockInstanceId, block: BlockInstance) -> Either[DataPartitionLookup, Error]:  # type: ignore[invalid-argument] # semigroup
    ...
    match block.factory_id.factory:
        case "exampleSource":
            action = from_source(np.array(["fiab_plugin_toy_impl.datasource.from_example"]))
        case "ekdSource":
            request_params = {
                "param": ["2t", "msl"],
                "levtype": "sfc",
            ...
            action = from_source(
                np.array(
                    [
                        Payload(
                            "fiab_plugin_toy_impl.datasource.from_source",
        ...
        case "meanProduct":
            ...
            action = input_task_action.map(
                Payload("fiab_plugin_toy_impl.product.select", kwargs={"variable": block.configuration_values["variable"]})
            ).map(Payload("fiab_plugin_toy_impl.product.mean"))
        case "dummySink":
            input_task = block.input_ids["dataset"]
