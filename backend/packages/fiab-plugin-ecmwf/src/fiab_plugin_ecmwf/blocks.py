...
class GenericSelectDimension(Transform):
    title: str = "Generic Select Dimension"
    description: str = "Select any dimension from the input dataset"
    inputs: list[str] = ["dataset"]
    configuration_options: dict[ConfigurationOptionId, BlockConfigurationOption] = {
        ConfigurationOptionId("dimension"): BlockConfigurationOption(
            title="Dimension",
            description="Dimension to select from the dataset",
            value_type="str",
        ),
        ConfigurationOptionId("values"): BlockConfigurationOption(
            title="Values",
            description="Values to select from the dataset dimension",
            value_type="list[str]",
        ),
    }
    ...
    def validate(self, block: BlockInstance, inputs: dict[str, QubedOutput]) -> Either[BlockInstanceOutput, Error]:  # type:ignore[invalid-argument] # semigroup
        ...
        dimension = block.config_as_str(ConfigurationOptionId("dimension"))
        values = block.config_as_list(ConfigurationOptionId("values"), str, allow_empty=False)

        if dimension not in dimensions(input_dataset):
            return Either.error(f"Dimension '{dimension}' is not in the input dataset dimensions: {dimensions(input_dataset)}")

        type_in_input = type(next(iter(axes(input_dataset)[dimension])))
        typed_values = list(map(type_in_input, values))

        if not contains(input_dataset, {dimension: typed_values}):
            return Either.error(f"Values {typed_values} are not in the input {dimension}: {axes(input_dataset).get(dimension, [])}")

        output = coxpand(input_dataset, dimension, {dimension: values})
        return Either.ok(output)
    ...
    def restrictions(self, other: QubedOutput) -> ConfigurationOptionRestriction:
        dimensions_other = dimensions(other)
        return (
            {
                ConfigurationOptionId("dimension"): ClosedEnumType(list(dimensions_other)),
                ConfigurationOptionId("values"): ListType(
                    ClosedEnumType(list(str(value) for dim in dimensions_other for value in axes(other).get(dim, set())))
                ),
            }
            if dimensions_other
            else {}
