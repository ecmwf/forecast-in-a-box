...
2. Inspect the codebase in `backend/src/*` and `backend/packages/*/src` for where it is conflicting with the guidelines. Do not make changes to the codebase itself, only find where there is misalignment. You can spawn subagents for individual submodules and parts of the guide.
