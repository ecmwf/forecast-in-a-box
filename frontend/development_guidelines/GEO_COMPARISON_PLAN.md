...
- [x] **I1 — Sidebar restructure** *(first — biggest usability win)*:
  ...
  groups with level entries, surface/pressure sections, level filter chips),
  search, and an `All | A | B` availability filter. Effective opacity =
...
browsing (token-based Copernicus presets later); more feedback rounds
expected.
