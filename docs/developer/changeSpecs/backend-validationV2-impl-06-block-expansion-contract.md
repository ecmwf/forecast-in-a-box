...
- Update backend aggregation in `validate_expand` so `/blueprint/expand` can still identify the plugin for each expansion.
...
- The route-level response must not lose plugin identity. Core `BlockExpansion` may contain a local `BlockFactoryId`, but the HTTP response needs enough data for clients to instantiate the correct plugin block.
